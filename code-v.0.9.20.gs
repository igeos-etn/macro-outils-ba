...
const VERSION_SCRIPT = "0.9.20"; // Ajout de la rotation des logs
const AGENCE_ID = "AG02"; // <--- MODIFIEZ ICI LE NUMERO D'AGENCE
const AUTHORIZED_GROUP = "transfert-boite-agence-02@etn.fr";
  ...
  if (threads.length > 0) {
    ...
    for (const thread of threads) {
      ...
      for (const message of messages) {
        ...
        let corpsOriginal = message.getBody();
        
        // CORRECTION 1 : Purger les images encodées directement en Base64 qui font planter le htmlBody
        corpsOriginal = corpsOriginal.replace(/src=["']data:image\/[^;]+;base64,([^"']+)["']/gi, function(match, base64data) {
          if (base64data.length > 30000) { // Si l'image fait plus de ~20 Ko en texte base64
            return 'src="#" alt="[Image intégrée trop lourde supprimée pour le transfert]" style="border:1px dashed #ccc; padding:5px;"';
          }
          return match;
        });
        ...
        // CORRECTION 2 : Extraire une adresse email pure pour "replyTo" (évite l'erreur "Invalid argument: replyTo")
        let safeReplyTo = from;
        const emailMatch = from.match(/<([^>]+)>/);
        if (emailMatch && emailMatch[1]) {
          safeReplyTo = emailMatch[1];
        }
          ...
          if (analyse.isBodySizeError) {
            ...
            try {
              ...
              GmailApp.sendEmail(ADRESSE_DESTINATAIRE, nouveauSujet + " [Format .eml]", "Voir liens Drive.", {
                htmlBody: corpsAlerteUser, replyTo: safeReplyTo
              });
...
function getInlineImagesFromRaw(message) {
  ...
  parts.forEach(part => {
    // CORRECTION 3 : Ne scanner que les 1500 premiers caractères pour éviter de bloquer la RAM sur les gros fichiers
    const headerBlock = part.substring(0, 1500).split(/\r?\n\r?\n/)[0];
  ...
  attachments.forEach(pj => {
    ...
    if (cid && referencedCids[cid]) {
      inlineMap[cid] = pj; // CORRECTION 4 : On utilise l'objet 'pj' natif au lieu de le reconstruire
    } else {
      regularAttachments.push(pj); // Evite de crasher sur pj.getBytes()
    }
  });
...
function execeptionMsg(e) {
  ...
  } else if ((errStr.includes("body") && errStr.includes("size")) || (errStr.includes("taille") && errStr.includes("corps")) || errStr.includes("invalid argument: htmlbody") || errStr.includes("argument invalide : htmlbody")) {
    // Le "invalid argument" est souvent déclenché quand le htmlBody est saturé par du texte Base64
    res.messageClair = "Non transfere : Contenu mail trop volumineux ou invalide";
