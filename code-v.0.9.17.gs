...
const VERSION_SCRIPT = "0.9.17"; // Mise à jour de la version
...
// ==============================================================================
// 5. GESTION DES LOGS (OPTIMISÉ)
// ==============================================================================

// Variables globales pour mémoriser la feuille de log pendant l'exécution
...
function getLogSpreadsheet() {
  ...
  // Si on a déjà sauvegardé l'ID, on essaie d'ouvrir directement le fichier
  if (ssId) {
    try {
      return SpreadsheetApp.openById(ssId);
    } catch(e) {
      // Si le fichier a été supprimé ou est inaccessible, on efface la mémoire
      props.deleteProperty('LOG_SS_ID');
    }
  }
  
  // Sinon, on fait une recherche Drive (beaucoup plus long)
...
function getGlobalLogSheet() {
  // Si la feuille est déjà chargée en mémoire dans ce script, on la renvoie direct
  ...
  // Mémorisation pour les prochains appels
...
function writeLog(level, msg) {
  ...
  try { 
    getGlobalLogSheet().appendRow(row); 
  } catch (e) { 
...
function isUserAuthorized(email) {
  ...
  // 1. Si on a déjà vérifié cet utilisateur récemment, on lit le cache (0 appel API)
